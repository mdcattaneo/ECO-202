...
% Generic pacing defaults; an individual paper may override these in its source.
\newcommand{\examtiming}{Target workload: 70 minutes (10 minutes multiple choice; 60 minutes written work). Follow announced start/end times within the 80-minute meeting. For practice, set a 70-minute timer.}
\newcommand{\writtenminutes}{15}
...
\newcommand{\mcq}[5]{%
  ...
  \begin{enumerate}[label=\textbf{\Alph*.},leftmargin=3.8em,labelsep=0.65em,itemsep=1pt,topsep=3pt]
  \item #2
  \item #3
  \item #4
  \item #5
...
\newcommand{\written}[2]{%
  \clearpage
  \lhead{ECO 202 \textbar\ Fall 2026 \textbar\ Question #1}%
  \section*{#1. #2 \normalfont\large(20 points; about \writtenminutes\ minutes)}%
}
\newlist{parts}{enumerate}{1}
\setlist[parts]{label=\textbf{(\alph*)},leftmargin=2em,itemsep=7pt,topsep=5pt}
\newcommand{\pts}[1]{\textbf{(#1 points)}\ }
% Double original allocations, with at least 2.5 inches for every written part.
\newlength{\answerspace}
\newcommand{\workspace}[1]{%
  \setlength{\answerspace}{\dimexpr#1+#1\relax}%
  \ifdim\answerspace<2.5in\setlength{\answerspace}{2.5in}\fi
  \par\vspace*{\answerspace}%
}
% Keep each written prompt and its full workspace together across page breaks.
% Arguments: original workspace height, prompt (including its point value).
